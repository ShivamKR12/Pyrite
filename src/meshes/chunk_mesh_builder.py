...
def get_vertex_light(local_vertex_pos: Tuple[int, int, int], world_vertex_pos: Tuple[int, int, int], plane: str, face_light: int, chunk_voxels: Any, chunk_lightmap: Any, world_voxels: Any, world_lightmaps: Any, chunk_positions: Any) -> int:
    ...
    if plane == 'Y':
        ...
        b3 = get_neighbor_voxel_id((lx - 1, ly, lz - 1), (vx - 1, vy, vz - 1), chunk_voxels, world_voxels, chunk_positions)
        
        l0 = face_light if not is_transparent(b0) else get_neighbor_light((lx, ly, lz), (vx, vy, vz), chunk_lightmap, world_lightmaps, chunk_positions)
        l1 = face_light if not is_transparent(b1) else get_neighbor_light((lx - 1, ly, lz), (vx - 1, vy, vz), chunk_lightmap, world_lightmaps, chunk_positions)
        l2 = face_light if not is_transparent(b2) else get_neighbor_light((lx, ly, lz - 1), (vx, vy, vz - 1), chunk_lightmap, world_lightmaps, chunk_positions)
        l3 = face_light if not is_transparent(b3) else get_neighbor_light((lx - 1, ly, lz - 1), (vx - 1, vy, vz - 1), chunk_lightmap, world_lightmaps, chunk_positions)
        ...
        b2 = get_neighbor_voxel_id((lx, ly, lz - 1), (vx, vy, vz - 1), chunk_voxels, world_voxels, chunk_positions)
        b3 = get_neighbor_voxel_id((lx, ly - 1, lz - 1), (vx, vy - 1, vz - 1), chunk_voxels, world_voxels, chunk_positions)
        
        l0 = face_light if not is_transparent(b0) else get_neighbor_light((lx, ly, lz), (vx, vy, vz), chunk_lightmap, world_lightmaps, chunk_positions)
        l1 = face_light if not is_transparent(b1) else get_neighbor_light((lx, ly - 1, lz), (vx, vy - 1, vz), chunk_lightmap, world_lightmaps, chunk_positions)
        l2 = face_light if not is_transparent(b2) else get_neighbor_light((lx, ly, lz - 1), (vx, vy, vz - 1), chunk_lightmap, world_lightmaps, chunk_positions)
        l3 = face_light if not is_transparent(b3) else get_neighbor_light((lx, ly - 1, lz - 1), (vx, vy - 1, vz - 1), chunk_lightmap, world_lightmaps, chunk_positions)
        ...
        b3 = get_neighbor_voxel_id((lx - 1, ly - 1, lz), (vx - 1, vy - 1, vz), chunk_voxels, world_voxels, chunk_positions)
        
        l0 = face_light if not is_transparent(b0) else get_neighbor_light((lx, ly, lz), (vx, vy, vz), chunk_lightmap, world_lightmaps, chunk_positions)
        l1 = face_light if not is_transparent(b1) else get_neighbor_light((lx - 1, ly, lz), (vx - 1, vy, vz), chunk_lightmap, world_lightmaps, chunk_positions)
        l2 = face_light if not is_transparent(b2) else get_neighbor_light((lx, ly - 1, lz), (vx, vy - 1, vz), chunk_lightmap, world_lightmaps, chunk_positions)
        l3 = face_light if not is_transparent(b3) else get_neighbor_light((lx - 1, ly - 1, lz), (vx - 1, vy - 1, vz), chunk_lightmap, world_lightmaps, chunk_positions)
    ...
    for y in range(CHUNK_SIZE):
        ...
        for x in range(CHUNK_SIZE):
            ...
            for z in range(CHUNK_SIZE):
                ...
                # top face
                neighbor_id = get_neighbor_voxel_id((x, y + 1, z), (wx, wy + 1, wz), chunk_voxels, world_voxels, chunk_positions)
                ...
                if is_transparent(neighbor_id) and voxel_id != neighbor_id:
                    ...
                    face_light = get_neighbor_light((x, y + 1, z), (wx, wy + 1, wz), chunk_lightmap, world_lightmaps, chunk_positions)
                    l0 = get_vertex_light((x    , y + 1, z    ), (wx    , wy + 1, wz    ), 'Y', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l1 = get_vertex_light((x + 1, y + 1, z    ), (wx + 1, wy + 1, wz    ), 'Y', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l2 = get_vertex_light((x + 1, y + 1, z + 1), (wx + 1, wy + 1, wz + 1), 'Y', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l3 = get_vertex_light((x    , y + 1, z + 1), (wx    , wy + 1, wz + 1), 'Y', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                ...
                if is_transparent(neighbor_id) and voxel_id != neighbor_id:
                    ...
                    face_light = get_neighbor_light((x, y - 1, z), (wx, wy - 1, wz), chunk_lightmap, world_lightmaps, chunk_positions)
                    l0 = get_vertex_light((x    , y, z    ), (wx    , wy, wz    ), 'Y', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l1 = get_vertex_light((x + 1, y, z    ), (wx + 1, wy, wz    ), 'Y', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l2 = get_vertex_light((x + 1, y, z + 1), (wx + 1, wy, wz + 1), 'Y', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l3 = get_vertex_light((x    , y, z + 1), (wx    , wy, wz + 1), 'Y', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
    ...
    for x in range(CHUNK_SIZE):
        ...
        for y in range(CHUNK_SIZE):
            ...
            for z in range(CHUNK_SIZE):
                ...
                neighbor_id = get_neighbor_voxel_id((x + 1, y, z), (wx + 1, wy, wz), chunk_voxels, world_voxels, chunk_positions)
                ...
                if is_transparent(neighbor_id) and voxel_id != neighbor_id:
                    ...
                    face_light = get_neighbor_light((x + 1, y, z), (wx + 1, wy, wz), chunk_lightmap, world_lightmaps, chunk_positions)
                    l0 = get_vertex_light((x + 1, y    , z    ), (wx + 1, wy    , wz    ), 'X', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l1 = get_vertex_light((x + 1, y + 1, z    ), (wx + 1, wy + 1, wz    ), 'X', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l2 = get_vertex_light((x + 1, y + 1, z + 1), (wx + 1, wy + 1, wz + 1), 'X', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l3 = get_vertex_light((x + 1, y    , z + 1), (wx + 1, wy    , wz + 1), 'X', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                ...
                if is_transparent(neighbor_id) and voxel_id != neighbor_id:
                    ...
                    face_light = get_neighbor_light((x - 1, y, z), (wx - 1, wy, wz), chunk_lightmap, world_lightmaps, chunk_positions)
                    l0 = get_vertex_light((x, y    , z    ), (wx, wy    , wz    ), 'X', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l1 = get_vertex_light((x, y + 1, z    ), (wx, wy + 1, wz    ), 'X', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l2 = get_vertex_light((x, y + 1, z + 1), (wx, wy + 1, wz + 1), 'X', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l3 = get_vertex_light((x, y    , z + 1), (wx, wy    , wz + 1), 'X', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
    ...
    for z in range(CHUNK_SIZE):
        ...
        for x in range(CHUNK_SIZE):
            ...
            for y in range(CHUNK_SIZE):
                ...
                neighbor_id = get_neighbor_voxel_id((x, y, z - 1), (wx, wy, wz - 1), chunk_voxels, world_voxels, chunk_positions)
                ...
                if is_transparent(neighbor_id) and voxel_id != neighbor_id:
                    ...
                    face_light = get_neighbor_light((x, y, z - 1), (wx, wy, wz - 1), chunk_lightmap, world_lightmaps, chunk_positions)
                    l0 = get_vertex_light((x    , y    , z), (wx    , wy    , wz), 'Z', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l1 = get_vertex_light((x    , y + 1, z), (wx    , wy + 1, wz), 'Z', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l2 = get_vertex_light((x + 1, y + 1, z), (wx + 1, wy + 1, wz), 'Z', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l3 = get_vertex_light((x + 1, y    , z), (wx + 1, wy    , wz), 'Z', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                ...
                if is_transparent(neighbor_id) and voxel_id != neighbor_id:
                    ...
                    face_light = get_neighbor_light((x, y, z + 1), (wx, wy, wz + 1), chunk_lightmap, world_lightmaps, chunk_positions)
                    l0 = get_vertex_light((x    , y    , z + 1), (wx    , wy    , wz + 1), 'Z', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l1 = get_vertex_light((x    , y + 1, z + 1), (wx    , wy + 1, wz + 1), 'Z', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l2 = get_vertex_light((x + 1, y + 1, z + 1), (wx + 1, wy + 1, wz + 1), 'Z', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
                    l3 = get_vertex_light((x + 1, y    , z + 1), (wx + 1, wy    , wz + 1), 'Z', face_light, chunk_voxels, chunk_lightmap, world_voxels, world_lightmaps, chunk_positions)
