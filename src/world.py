...
class World:
    ...
    def update(self) -> None:
        ...
        if self.db_load_time > 0 or self.terrain_gen_time > 0:
            print(
                f'[FRAME TIME] Chunk Loading -> DB Read: {self.db_load_time:.4f}s | Terrain Gen: {self.terrain_gen_time:.4f}s'
            )
        ...
        if self.build_queue:
            ...
            if getattr(self, '_last_sort_pos', None) != (player_cx, player_cz) or getattr(
                self, '_last_queue_len', None
            ) != len(self.build_queue):
                self.build_queue.sort(
                    key=lambda c: (c.position[0] - player_cx) ** 2 + (c.position[2] - player_cz) ** 2, reverse=True
                )
    ...
    def process_mesh_queue(self) -> None:
        ...
        for item in list(self.mesh_queue):
            ...
            if future.done():
                ...
                limit = (
                    MAIN_THREAD_MESH_PROCESS_LIMIT_LOADING
                    if self.app.game_state == 'LOADING'
                    else MAIN_THREAD_MESH_PROCESS_LIMIT_INGAME
                )
        ...
        for item in list(self.load_queue):
            ...
            if future.done():
                ...
                limit = (
                    MAIN_THREAD_CHUNK_PROCESS_LIMIT_LOADING
                    if self.app.game_state == 'LOADING'
                    else MAIN_THREAD_CHUNK_PROCESS_LIMIT_INGAME
                )
    ...
    def _fetch_or_generate_voxels(
        self, x: int, y: int, z: int
    ) -> Tuple[str, float, NDArray[np.uint8], NDArray[np.uint8], bool, bool]:
        """
    ...
    def save_chunk_to_db(
        self, x: int, y: int, z: int, voxels: NDArray[np.uint8], lightmap: Optional[NDArray[np.uint8]]
    ) -> None:
        """
