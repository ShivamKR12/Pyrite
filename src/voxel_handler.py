...
from settings import (
    CHUNK_SIZE, CHUNK_AREA, MAX_RAY_DIST, NON_PLACEABLE, GLOWSTONE, STONE, 
    WOODEN_PICKAXE, WATER, SURVIVAL
)
...
class VoxelHandler:
    ...
    def add_voxel(self) -> None:
        ...
        if self.voxel_id:
            ...
            if not result[0]:
                ...
                def async_add_voxel(wx: float = wx, wy: float = wy, wz: float = wz, cid: int = current_id, ch: Any = chunk) -> None:
    ...
    def remove_voxel(self) -> None:
        ...
        if self.voxel_id:
            ...
            def async_remove_voxel(wx: float = wx, wy: float = wy, wz: float = wz, vid: int = self.voxel_id, ch: Any = self.chunk) -> None:
                if vid == GLOWSTONE:
                    update_light_place_block(int(wx), int(wy), int(wz), self.app.scene.world.voxels, self.app.scene.world.lightmaps, self.app.scene.world.chunk_positions)
