        ...
        for file in files:
            ...
            if file not in ALLOWED_PARALLEL_MODULES:
                with open(os.path.join(root, file), 'r', encoding='utf-8') as f:
                    content = f.read()
                    assert 'parallel=True' not in content, f"Numba Constraint Violation: '{file}' uses 'parallel=True'. This is not allowed!"
                    assert 'prange' not in content, f"Numba Constraint Violation: '{file}' imports or uses 'prange'. This is not allowed!"